...
def create_ui():
    ...
    with gr.Blocks(title="音频质量评估系统", css=css) as demo:
        ...
        with gr.Column(visible=True) as info_col:
            ...
            note_text = gr.Markdown(
                value="""
                    ...
                    ### 2. 空间感知评估（后四个音频）
                    • 主要关注个人空间感知情况，后续正式的音频空间感评估将只有声源一致性评估，将更加简单\n
                    • 距离：表示声音来源距离（0-10米）\n
                    ...
                    ### 三、正式评估
                    • 在音频评估页面，请认真听取每个音频并给出音质评分以及声源一致性评分（通过图像判断音频是否来自这一方向）！\n
                    • 声源一致性评分范围：1-5分（1分为根本不是这个方向，5分为方向是完全匹配的）
        ...
        with gr.Column(visible=False) as audio_test_col:
            ...
            #     <h4>空间感知评估说明</h4>
            #     <ul>
            #         <li>主要关注个人空间感知情况，后续正式的音频空间感评估将只有声源一致性评估，将更加简单</li>
